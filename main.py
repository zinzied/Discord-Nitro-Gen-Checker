...
class TerminalTitleSetter:
    ...
    def set_title(checked_count=None, elapsed_time=None):
        title = "By: ZIEDEV | Discord Gift Checker 2k24|"
...
class GiftCheckerApp:
    ...
    def check_codes(s, CA):
        ...
        # Generate new codes and overwrite the codes file
        new_codes = CodeGenerator.generate_amount(CA)
        FileHandler.overwrite(new_codes, s.codes_file)

        with cf.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_code = {executor.submit(s.checker.check_code, code): code for code in new_codes}
            with tqdm(total=CA, desc="Checking Codes", unit="code") as pbar:
